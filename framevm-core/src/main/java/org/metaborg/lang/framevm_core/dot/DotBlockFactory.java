...
public class DotBlockFactory extends DotFactory {
	...
	private final static String[] REGEX_UNPACK = {"Terminal\\((.+)\\)", "\"([^\"]*)\"", "Bind\\((\\w+),\\d+\\)", "FVM_Link\\((\\w+)\\)", "FVM_Slot\\((\\w+)\\)", "FVM_Cont\\((\\w+)\\)", "FVM_Path\\((\\[.+\\])\\)"};
	...
	private final static Pattern RGR_PREFIX_PATTERN = Pattern.compile("RGR_");
	...
	private static String sanitize(String instr, int idx, String block, List<String> links) {
		...
		out = RGR_PREFIX_PATTERN.matcher(out).replaceAll("");
